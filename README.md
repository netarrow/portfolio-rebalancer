...
- **Transfer cost** — what this account charges to wire money **out** of it: free, a flat fee, or a percentage with an optional floor and ceiling. The fee belongs to the sender, so it is read from the account the money leaves — an everyday current account is a broker like any other here, with no commission plan and a SEPA fee on what it sends. The [YNAB funding plan](#funding-plan) prices every wire with it and checks the account can actually afford to send it.
...
- Each category says where its money **starts** — the *From* account, a broker like any other — and where it ends up: an **asset** (its money becomes shares), a **whole portfolio** (see below), or a **broker's cash** (it stays liquid there). An asset mapping can also name the **broker** the order goes through and the **portfolio** it belongs to. Categories with no account of their own use the plan's default one, set once under *Wire from*.
...
- **A whole portfolio as the destination** answers "this money is for the Main Strategy" without naming the instrument: the portfolio's own targets decide what to buy. A [percent-mode](#portfolios) portfolio spreads the money over its underweight rows, proportionally to each one's gap — the same buy-only distribution Asset Allocation and Fund Relocation use — while an [amount-mode](#targets-in--goal-matching) one fills its € targets **nearest due date first**, in whole bond lots. Nothing is ever sold, a row never receives more than its own gap, and the split hands over euro rather than share counts, so the only rounding is the commission-aware one downstream. Each slice then becomes an ordinary order: same commission, same lot sizing, same free-buy check, with its *Funded by* line naming the portfolio that split it.
...
- **Where the money comes from**: every wire names the account it leaves, and is priced with **that** account's [transfer cost](#brokers) — the sender pays the bank, not the destination. A broker funded by two different accounts shows both legs with their own amounts and fees. A fee that eats more than the flag threshold of its wire is marked *costly wire*, the hint that one bigger transfer beats repeating small ones.
- **Leaving your accounts** is the mirror image: per sending account, what goes out, what the bank charges, the free cash it holds (net of its minimum liquidity and of what it has earmarked) and what is left afterwards — flagged *short* when the plan asks for more than it has, before any money moves. *Register purchases* then moves both ends: the wire out of the sender with its fee, and into the broker that receives it.
